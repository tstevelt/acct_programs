/*----------------------------------------------------------------------------
	Program : ar_statement.c
	Author  : Tom Stevelt
	Date    : 2000-2024
	Synopsis: A/R Print Statement
...
int main ( int argc, char *argv[] )
{
	...
	{
		case MODE_START:
			StartUp ( "accounting - print statement", SessionRecord.DbName, 1, 1, 1, "javascript:Loaded();", AR_MENU );

			ptrWebParms = webInitParms ();

			ptrWebParms->WP_FormName   = "rptform";
			ptrWebParms->WP_FormAction = "ar_statement.cgi";
		...
		case MODE_RUN:
			ReportUp ( "print statement", SessionRecord.DbName, "javascript:Loaded();", ReportFormat, "chart" );
			break;
	}
		...
		case MODE_RUN:
			...
			if ( getdata () > 0 )
			{
				...
				else
				{
					...
					sprintf ( Subject, "Statement for %s", xarcust.xname );
					Attachments[0] = fnPDF;
